...
namespace izumo::core {
    ...
    ev_loop::instance()
    {
	static thread_local ev_loop_epoll ret;
	return ret;
    }
    ...
    void
    ev_loop_epoll::add_watcher(ev_watcher &watcher) 
    {
	epoll_event ev;
    ...
    void
    ev_loop_epoll::remove_watcher(ev_watcher& watcher)
    {
    ...
    void
    ev_loop_epoll::add_timer(ev_watcher &watcher, timedelta_ms_t timeout)
    {
	...
	if (m_timeout_queue.size() != 0) {
	    auto now = clock::now();
	    timeout = std::max(static_cast<int>(m_timeout_queue.top().deadline) - static_cast<int>(now), 0);
	}

	int ret = epoll_wait(m_epfd, evs, 128, timeout);

	if (ret < 0) {
	    if (errno != EINTR) throw osexception();
	    return;
	}
